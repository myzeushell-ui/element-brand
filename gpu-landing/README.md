...
1. **Фотографии применений** — сейчас реальные снимки с Pexels (лицензия для
   коммерческого использования, атрибуция не требуется; ID в
   `public/images/apps/CREDITS.md`). При желании заменить на собственные фото объектов —
   положить в `public/images/apps/` под теми же именами (`factory/oil/crane/server/medical.jpg`).
